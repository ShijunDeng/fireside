...
- “systemd-run 成功 exec”不等于 watchdog 已就绪。watchdog 必须使用 txid 绑定的 `Type=notify`（或同强度握手），在验证固定生产控制器、txid、已存在且匹配的 prepared journal、锁 owner/mode/link/inode且已准备阻塞等待该锁后才向 PID 1 发 READY；controller 有界等待 READY 并复查 unit active 后才能继续 active/revoke。READY 由短生命周期 `systemd-notify` helper 发送时，必须使用 `--pid=parent` 在载荷中绑定父脚本 PID，并对这个只执行固定、root-owned 控制器且无网络/业务密钥访问的 transient unit 设置 `NotifyAccess=all`，使缺少 PID 伪装 capability 后回退为 helper 凭据的同步通知仍被接收；不得为此增加 `CAP_SYS_ADMIN`，不得依赖可能被 `systemd-run` 参数展开改写的 shell PID 字面量。exec 后立即退出、坏参数/锁/journal、未 READY、超时或 unit 被杀均在切换前失败关闭；异常退出由 PID 1 重启，正常看到 journal 已清后成功退出。
- watchdog 的 systemd sandbox 只保留恢复主库和所有权所需的最小 `CAP_DAC_OVERRIDE/CAP_CHOWN/CAP_FOWNER`，不保留完整 root capability；网络只允许恢复健康检查必需的 localhost，必须显式拒绝其他所有 IP；业务密钥与只在 install 使用的 GitHub deploy key 目录都必须不可见。工作路径固定且不能被调用环境覆盖。
...
- systemd 单元、`fireside`/`fireside-build` 身份、root-only 密钥、`/var/lib/fireside-release`、生产状态和备份目录是 bootstrap 的显式前置条件。生产运行手册必须先安装/加载单元并让 recovery 进入本次开机的 active-exited 状态，再执行 install/bootstrap；不能先 promote、后安装单元。
...
25. 干净 fixture 按生产运行手册从创建目录、安装/加载 unit、install 到 `bootstrap` 后在 80 端口健康；初始没有 current/previous/DB，成功后 current 指向目标、previous 仍不存在、主库私有且 status 为 clean。
...
55. 在无 `/var/lib/fireside-release` 的干净主机根按生产运行手册逐条执行；准备阶段必须先创建 root:root0700 状态目录，recovery 首次 start 返回0并 active(exited)，随后 install/bootstrap/80 健康。删除该目录后，前置验收必须明确失败，不得对缺失 namespace 路径宣称可部署。
...
64. 用真实 systemd transient `Type=notify` watchdog，在 controller 持有主锁时执行 prepared→READY 握手；`systemd-notify --pid=parent` 载荷绑定 unit main 脚本 PID，helper 通知被接收后启动命令在有界秒数内返回且 unit 保持 active/waiting。随后杀 controller supervisor，watchdog立即取得锁并恢复 prepared journal；不得出现 systemd-run等待unit退出、watchdog等待主锁的双向死锁。
65. 用与生产相同的 dispatcher→promote 脚本→`systemd-notify` helper→`flock` 进程链、最小 capability bounding set 和真实持锁场景执行 acceptance 64；unit 必须显式 `NotifyAccess=all`，且 bounding set 不含 `CAP_SYS_ADMIN`。journal 显示 prepared 时 `systemd-run` 在 5 秒内返回 0、unit 为 active、MainPID保持父脚本/后继 `flock` PID；释放主锁后完成恢复并清理 unit。`NotifyAccess=main` 必须作为反例确定失败，不能用直接以 bash 为 main 的简化探针替代生产链。
66. 检查真实 watchdog transient unit：`IPAddressDeny=any` 与 `IPAddressAllow=localhost` 同时生效，unit 内可请求宿主 `127.0.0.1` 健康接口但访问任一非回环宿主地址失败；`InaccessiblePaths` 同时覆盖 `/etc/fireside.env` 与整个 `/etc/fireside-release`。考虑恢复所需 `CAP_DAC_OVERRIDE` 可让 root 打开 mode 000 的 systemd 空占位，验收判据必须是业务口令路径呈现 size 0 的空文件、deploy key 与 known_hosts 在空目录下均不存在，严禁用 `-r` 误判，更严禁读取或输出原始内容。上述约束不能影响 systemd notify、锁等待、restart、回环健康检查与 prepared journal 恢复。
...
AV 首修的简化探针新增阻断 AW：直接由 systemd 启动 bash 时 `--pid=parent` 可被正确归属，但生产 watchdog 的 capability bounding set 不允许 helper 伪装父 PID，`systemd-notify` 按文档回退为自身凭据，`NotifyAccess=main` 仍拒绝。该只执行固定 root-owned 控制器、无网络且不可读业务密钥的 unit 必须改为 `NotifyAccess=all`，保留同步通知与父 PID 载荷且不得增加 `CAP_SYS_ADMIN`；真实生产等价进程链探针必须取代过度简化的假阳性。该恢复可用性 P1 使成熟度计数继续为 0。

AW 等价 sandbox 复核新增阻断 AX：既有规范要求 watchdog 无公网和生产密钥访问，但 transient unit 只隐藏 `/etc/fireside.env`，没有隐藏 `/etc/fireside-release` 的 GitHub deploy key，也没有网络出站限制。恢复健康检查需要宿主 loopback，故必须采用 `IPAddressDeny=any` + `IPAddressAllow=localhost`，并隐藏整个 deploy-key 目录；真实 unit 同时验证回环可用、公网不可用与两类密钥不可见。该纵深防御 P1 使成熟度计数继续为 0。
