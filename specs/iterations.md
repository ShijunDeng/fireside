...
## Iteration 095：README 回到产品与业务旅程（Accepted）

- 规格：SPEC-041；根 README 虽有功能清单，但缺少角色、两种发布路径、口令介入时机、生命周期纠错和同日多场策略，且生产安装细节占据主体。
- 决策：根 README 重构为产品定位、操作逻辑、使用指南、能力边界和开发入口；完整生产内容迁入独立运行手册并保持可追溯链接。
- 验收：业务说明与 SPEC-011/018/019 和实际页面一致；本地命令、API 表及全部 Markdown 相对链接有效；`npm run check` 通过。本轮不修改业务代码或生产运行态。
- 结果：根 README 已覆盖角色、两种发布路径、口令介入、完整生命周期、纠错和同日多场；生产手册独立保留全部原内容。本轮 5 个 Markdown 文件相对链接全部有效，完整 check 181/181、TypeScript 与生产构建通过。
