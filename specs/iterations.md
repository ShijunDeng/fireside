...
## Iteration 021：认证抗猜测、短期协作会话与生产密钥强度（Ready）
...
- 状态：Ready；必须先实现并完成自动化、跨端与生产验收，再更新为 Accepted。
- 发布前审查补充：已确定性发现原始 URL 正则可被百分号编码的敏感 GET 绕过、HEAD 未守卫，以及退出后迟到会议响应可重新暴露链接；另确认启动时不能仅凭 storage 乐观解锁。上述发现均已并入 SPEC-009，修复和新增回归完成前禁止验收，连续成熟度计数保持 0。
